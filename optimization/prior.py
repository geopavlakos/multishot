...
def create_prior(prior_type, **kwargs):
    ...
    elif prior_type == 'motion_difference':
        return DifferenceMotionPrior(**kwargs)
...
class DifferenceMotionPrior(nn.Module):
    def __init__(self, dtype=torch.float32, penalty='l2', **kwargs):
        super(DifferenceMotionPrior, self).__init__()
        if penalty == 'l1':
            self.loss_fn = nn.L1Loss()
        elif penalty == 'l2':
            self.loss_fn = nn.MSELoss()
        elif penalty == 'smoothl1':
            self.loss_fn = nn.SmoothL1Loss()
        elif penalty == 'elastic':
            loss_fn_l1 = nn.L1Loss()
            loss_fn_l2 = nn.MSELoss()
            self.loss_fn = lambda x,y: 0.5 * loss_fn_l1(x,y) + 0.5 * loss_fn_l2(x,y)

    def forward(self, x, motion_example_weight):
        if len(x) > 1:
            if len(x.shape) == 2:
                motion_example_weight = motion_example_weight[:, None]
            elif len(x.shape) == 3:
                motion_example_weight = motion_example_weight[:, None, None]
            return self.loss_fn(motion_example_weight*x[1:], motion_example_weight*x[:-1]).sum()
        else:
...
class MaxMixturePrior(nn.Module):
    ...
    def merged_log_likelihood(self, pose, betas):
        ...
        pose_dims = pose.shape[-1]
        diff_from_mean = pose.unsqueeze(dim=1) - self.means[: ,:pose_dims]

        prec_diff_prod = torch.einsum('mij,bmj->bmi',
                [self.precisions[:, :pose_dims, :pose_dims], diff_from_mean])
