...
def main(**args):
    ...
    motion_prior = create_prior(
        prior_type='motion_difference',
        penalty='l2',
        dtype=dtype,
        **args)
    ...
    for idx, data in enumerate(dataloader):
        ...
        cam_trans = data['cam_trans']*focal_length/5000
        ...
        curr_batch_size = keypoints.shape[0]
        if curr_batch_size < model_params['batch_size']:
            model_params['batch_size'] = curr_batch_size
            args['batch_size'] = curr_batch_size
            neutral_model = smplx.create(gender='neutral', **model_params).to(device)
        ...
        out_img_fn = [osp.join(curr_img_folder, 'frame_%05d.jpg' % (idx * batch_size + i)) for i in range(curr_batch_size)]
        mesh_fn = [osp.join(curr_mesh_folder, '%05d.obj' % (idx * batch_size + i)) for i in range(curr_batch_size)] 
        result_fn = osp.join(curr_result_folder, '%05d.pkl' % idx)

        fit_single_frame(img, keypoints,
                         height, width,
                         seqname, valid, shot,
