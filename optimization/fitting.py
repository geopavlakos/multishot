...
class SMPLifyLoss(nn.Module):
    ...
    def __init__(self, search_tree=None,
                 ...
                 motion_prior_body_pose_weight=1e+7,
                 motion_prior_betas_weight=1e+8,
                 motion_prior_canonical_joints_weight=1e+7,
                 reduction='sum',
                 **kwargs):
        ...
        self.register_buffer('motion_prior_body_pose_weight',
                             torch.tensor(motion_prior_body_pose_weight, dtype=dtype))
        self.register_buffer('motion_prior_betas_weight',
                             torch.tensor(motion_prior_betas_weight, dtype=dtype))
        self.register_buffer('motion_prior_canonical_joints_weight',
                             torch.tensor(motion_prior_canonical_joints_weight, dtype=dtype))
    ...
    def forward(self, body_model_output, camera, gt_joints, joints_conf,
        ...
        body_joints = body_model_output.joints
        projected_joints = camera(body_joints)
        ...
        joint_loss = torch.sum(torch.sum(weights ** 2 * joint_diff, dim=[1,2]) * self.data_weight ** 2)

        if self.motion_prior is None:
            motion_loss = torch.tensor(0., device=projected_joints.device)
        else:
            # smoothness for the pose and shape parameters
            motion_loss = self.motion_prior_body_pose_weight * self.motion_prior(body_model_output.body_pose.view(-1, pose_dims), motion_example_weight)
            motion_loss = motion_loss + self.motion_prior_betas_weight * self.motion_prior(body_model_output.betas.view(-1, 10), motion_example_weight)

            # joints in the canonical coordinate frame
            if self.motion_prior_canonical_joints_weight > 0:
                # bring joints back to canonical pose so that you can apply loss regardless of the shot change
                global_rotmat = batch_rodrigues(body_model_output.global_orient)
                # B x 3 x 3 -- B x J x 3 --> B x J x 3
                canonical_joints = torch.einsum('bti,bjt->bji', global_rotmat, body_joints)
                motion_loss = motion_loss + self.motion_prior_canonical_joints_weight * self.motion_prior(canonical_joints.view(-1, num_keyps, 3), motion_example_weight)
...
class SMPLifyCameraInitLoss(nn.Module):
    ...
    def forward(self, body_model_output, camera, gt_joints,
        ...
        joint_loss = torch.sum(torch.sum(joint_error, dim=[1,2]) * self.data_weight ** 2)
